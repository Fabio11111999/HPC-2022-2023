...
class Field:
    ...
    def exchange_startall(self):
        ...
        if domain.neighbour_north >= 0:
            self.north_recv_req = domain.comm.Irecv(self.bdryN, domain.neighbour_north, domain.rank)
            np.copyto(self._buffN, self._inner[:,-1])
            north_send_req = domain.comm.Isend(self._buffN, domain.neighbour_north, domain.neighbour_north)

        if domain.neighbour_south >= 0:
            self.south_recv_req = domain.comm.Irecv(self.bdryS, domain.neighbour_south, domain.rank)
            np.copyto(self._buffS, self._inner[:,0])
            south_send_req = domain.comm.Isend(self._buffS, domain.neighbour_south, domain.neighbour_south)

        if domain.neighbour_west >= 0:
            self.west_recv_req = domain.comm.Irecv(self.bdryW, domain.neighbour_west, domain.rank)
            np.copyto(self._buffW, self._inner[0,:])
            west_send_req = domain.comm.Isend(self._buffW, domain.neighbour_west, domain.neighbour_west)

        if domain.neighbour_east >= 0:
            self.east_recv_req = domain.comm.Irecv(self.bdryE, domain.neighbour_east, domain.rank)
            np.copyto(self._buffE, self._inner[-1,:])
            east_send_req = domain.comm.Isend(self._buffE, domain.neighbour_east, domain.neighbour_east)
    ...
    def exchange_waitall(self):
        ...
        domain = self._domain
        if domain.neighbour_north >= 0:
            self.north_recv_req.Wait()
        if domain.neighbour_south >= 0:
            self.south_recv_req.Wait()
        if domain.neighbour_west >= 0:
            self.west_recv_req.Wait()
        if domain.neighbour_east >= 0:
            self.east_recv_req.Wait()

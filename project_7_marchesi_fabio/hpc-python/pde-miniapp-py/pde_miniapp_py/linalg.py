...
def hpc_dot(x, y):
    ...
    ''' Slow Version
    res = 0.0
    for i in range(len(x._inner)):
        for j in range(len(x._inner[i])):
            res += x._inner[i][j] * y._inner[i][j]
    res = x.domain.comm.allreduce(res, op=MPI.SUM)
    return res
    '''

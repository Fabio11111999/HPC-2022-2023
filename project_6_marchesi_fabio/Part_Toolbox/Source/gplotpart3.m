function handle = gplotpart3(A,xy,part1,color1,color2,color3)
...
hold off
xy0 = [xy(:,1:2), zeros(size(xy, 1), 1)];
if length(part1) > 0 
    set(gplotg(A(part1,part1),xy0(part1,:),'-'),'color',color1);
    hold on
    set(scatter3(xy(part1,1),xy(part1,2),xy(part1,3), 10, xy(part1,3)));
    hold on
end;
if length(part2) > 0 
    set(gplotg(A(part2,part2),xy0(part2,:),'-'),'color',color2);
    hold on
    set(scatter3(xy(part2,1),xy(part2,2),xy(part2,3), 10, xy(part2,3)));
    hold on
end;
hold off